...
const options = [
  ...
  { name: "tools", type: "ToolSet", description: "Tools available to the model" },
];

export default function CreateAllemChatHandlerPage() {
  return (
    <div className="max-w-3xl">
      <h1 className="text-3xl font-bold tracking-tight">createAllemChatHandler</h1>
      ...
      <div className="mt-8 rounded-xl bg-neutral-900 p-4 ring-1 ring-white/10 shadow-lg">
        <code className="text-sm text-neutral-100">{`import { createAllemChatHandler } from "@allem-sdk/ai";`}</code>
      </div>
      ...
      <div className="mt-4 rounded-xl bg-neutral-900 p-4 ring-1 ring-white/10 shadow-lg">
        <pre className="text-sm text-neutral-100">{`// app/api/chat/route.ts
